...
for num_tokens in [512, 1024, 2048, 3600, -512, -1024, -2048, -3600, -1]:
    ...
    if num_tokens != -1:
        if num_tokens < -1:
            pd.DataFrame(all_data).to_parquet(f'/home/bingxing2/ailab/wangkuncan/soft/l1/deepscaler/data9_{num_tokens}/lsat.parquet')
        else:
            pd.DataFrame(all_data).to_parquet(f'/home/bingxing2/ailab/wangkuncan/soft/l1/deepscaler/data_{num_tokens}/lsat.parquet')
    else:
        pd.DataFrame(all_data).to_parquet(f'/home/bingxing2/ailab/wangkuncan/soft/l1/deepscaler/data/lsat.parquet')
